...
iTmax=12;iTmax=floor(iTmax/2);
n_hmax=20;
n_h_vec=[linspace(n_c,10,iTmax),linspace(10,n_hmax,iTmax)];
n_h_vec=unique(n_h_vec)
iTmax=length(n_h_vec);
figure
for iT=1:iTmax
    n_h=n_h_vec(1,iT);
    ...
    for ur=0:1
        if ur==0
            ...
            for i1=imin:imax
                ...
                Factorisation;
                tvec_dN1=jump_times;
                myVars2={"tvec_dN1","w_m",'w_hot','w_cold','w_cav',"n_h","T_c", "n_c",'Q_h','Q_h_f',...
...
figure
myVars0={'n_h_vec'};
sub_folder_name='n_h1';
load([sub_folder_name,'/in_cond_n_h1','traj1'],myVars0{:});%Just pick the n_h_vec from the first available mat file
for iT=1:iTmax
    n_h=n_h_vec(1,iT);
    sub_folder_name=['n_h',num2str(iT)];
    myVars = {'x_m_vec','p_m_vec'};
    load([sub_folder_name,'/in_cond_n_h',num2str(iT)],myVars{:})
    subplot(4,5,iT);
    plot(x_m_vec,1i*p_m_vec);
    title('nh=',num2str(n_h));
    [iT iTmax]
...
myVars0={'n_h_vec'};
sub_folder_name='n_h1';
load([sub_folder_name,'/in_cond_n_h1'],myVars0{:});%Just pick the n_c_vec from the first available mat file
sTmax=length(n_h_vec);
...
plot(n_h_vec,[-J_hot_vec/w_hot;J_cold_vec/w_cold;J_cav_vec/w_cav])
...
for iT=1:iTmax
    ...
    varvec=zeros(1,imax);
    for i1=imin:1:imax
        myVars = {"tvec_dN1",'w_m',"J_h",'n_h_vec'};
...
%%%Ent. Prod. From the ur=0 (unconditional evolution)
ent_prod=w_h*J_hot_vec.*(1./T_h_vec-1/T_c);
figure
semilogy(n_h_vec,ent_prod)
%Allan
% figure
...
for iT=1:iTmax
    sub_folder_name=['n_h',num2str(iT)];
    for i1=imin:1:imax
        myVars = {"tvec_dN1",'w_m','Q_h_f','n_h_vec'};
        load([sub_folder_name,'/in_cond_n_h',num2str(iT),'traj',num2str(i1)],myVars{:})
        %%%%This line will be passed only if you want to filter (detector dead time)
